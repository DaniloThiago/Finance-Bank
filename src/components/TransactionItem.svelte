<script lang="ts">
    ...
    const tiposTransacao = ['RECEBIMENTO PIX', 'PAGAMENTO PIX', 'TRANSFERÊNCIA BANCÁRIA', 'PAGAMENTO BOLETO', 'CARTÃO DE CRÉDITO'];
    const tipoTransacao = tiposTransacao[props.tipoTransacao];

    const status = props.status == 1 ? 'done' : 'clear';

    const year = props.data.substring(0, 4);
    const monthName = format(new Date(0, +props.data.substring(5, 7) - 1), 'LLL', { locale: ptBR });
    const day = props.data.substring(8, 10);
